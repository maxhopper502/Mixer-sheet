...
function clearCurrentJob() {
  ...
  if (confirm(`Clear all job and product data for ${aircraft}?`)) {
    ...
    alert(`Cleared all job data for ${aircraft}`);
  }
}

// Updated deleteProduct with correct keying and refresh
function deleteProduct(index) {
...
// Overwrite Clear Job again to also reload
function clearCurrentJob() {
  const f = new FormData(document.getElementById("job-form"));
  const aircraft = (f.get("aircraft") || "").trim().toUpperCase();
  if (!aircraft) {
  ...
  if (confirm(`Clear all job and product data for ${aircraft}?`)) {
    ...
    document.getElementById("job-form").reset();
    alert(`Cleared all job data for ${aircraft}`);
    location.reload();  // refresh to clear prefill
  }
}
