...
function editProduct(index) {
  ...
  const newRate = prompt("Edit Rate (e.g. 2.0):", product.rate);
  const newCount = prompt("Edit Container Count:", product.containers.length);
  const newSize = prompt("Edit Container Size:", product.containers[0]);
  if (newRate && newCount && newSize) {
    product.rate = parseFloat(newRate);
    product.containers = Array(parseInt(newCount)).fill(parseFloat(newSize));
