...
window.onload = function () {
  const mixerData = JSON.parse(localStorage.getItem("mixerData"));
  if (!mixerData || !mixerData.job || !mixerData.products || mixerData.products.length === 0) {
    alert("❌ No job or product found. Please import or set up a job first.");
    window.location.href = "setup.html";
    return;
  }

  const job = mixerData.job;
  const products = mixerData.products;
  document.title = `Mix ${job.aircraft}`;
