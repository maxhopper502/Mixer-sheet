document.addEventListener("DOMContentLoaded", function () {
  ...
  function addProduct(name, rate, unit, containers) {
    ...
    containerCol.appendChild(containerList);
    row.appendChild(containerCol);

    // Inline stocktake beside container list
    ...
    if (!isNaN(jobHa)) {
      ...
      stockCol.style.background = "#eef";
      stockCol.style.marginLeft = "30px";
      stockCol.style.padding = "6px";
      stockCol.style.borderRadius = "6px";
      stockCol.innerHTML = `
        <p><strong>📊 Stocktake:</strong><br>
        Req: ${required.toFixed(1)} ${unit}<br>
        Sup: ${supplied.toFixed(1)} ${unit}<br>
        Diff: ${diff.toFixed(1)} ${unit} ${icon}</p>
      `;
