...
function autoCalcRate() {
  ...
  if (!hectares || hectares <= 0) {
    document.getElementById("autoRate").textContent = "⚠️ Enter hectares first";
    return;
  }
  ...
  if (total > 0) {
    ...
    document.getElementById("autoRate").textContent = `Auto: ${rate.toFixed(3)} per ha`;
    document.getElementById("rateField").value = rate.toFixed(3);
  } else {
    document.getElementById("autoRate").textContent = "⚠️ Add container data";
  }
}

function attachAutoRateListeners() {
  const fields = ["container1", "count1", "container2", "count2", "hectares"];
  fields.forEach(name => {
...
document.getElementById("product-form").onsubmit = e => {
  ...
  let rate = parseFloat(data.get("rate"));
  ...
  if (!name || isNaN(rate) || containers.length === 0) {
    alert("❌ Please fill out all fields including a valid rate and containers.");
    return;
  }
