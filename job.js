...
function exportJob() {
  ...
  doc.text(`Pilot: ${job.pilot}`, marginLeft, y);
  y += 6;
  doc.text(`Aircraft: ${job.aircraft}`, marginLeft, y);
  y += 6;
  doc.text(`Mixer: ${mixer}`, marginLeft, y);
  y += 8;

  doc.text(`Crop: ${job.crop}`, marginLeft, y);
  y += 6;
  doc.text(`Hectares: ${job.hectares}`, marginLeft, y);
  y += 6;
  doc.text(`Vol/Ha: ${job.volPerHa}`, marginLeft, y);
  y += 6;
  doc.text(`Total Vol: ${Math.round(job.totalVolume)}`, marginLeft, y);
  y += 6;
  doc.text(`Load Area: ${job.loadArea.toFixed(1)} | Load Vol: ${Math.round(job.loadVolume)} | Loads: ${job.loads}`, marginLeft, y);
  y += 10;

  doc.setFontSize(12);
  doc.text("Products Used:", marginLeft, y);
  y += 6;
  products.forEach(p => {
  ...
  products.forEach((p, i) => {
    const containerState = p.containers || [];
    const remaining = containerState.reduce((a, b) => a + b, 0).toFixed(2);
  ...
  const cleanClient = (job.client || "client").replace(/\s+/g, "_");
  const cleanOrder = (job.orderNumber || "order").replace(/\s+/g, "_");
  doc.save(`${cleanClient}_${cleanOrder}_mixer_sheet.pdf`);
}
