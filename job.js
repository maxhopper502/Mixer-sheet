...
function showHeader() {
  ...
  el.innerHTML = `
    ...
    <p><strong>Vol/Ha:</strong> ${job.volPerHa.toFixed(2)} L &nbsp; <strong>Pilot:</strong> ${job.pilot} &nbsp; <strong>Aircraft:</strong> ${job.aircraft}</p>
  `;
}
  ...
  products.forEach((product, index) => {
    ...
    while (remaining > 0 && product.containers.length > 0) {
      ...
      if (available >= remaining) {
        ...
        fromContainers.push(`${remaining.toFixed(3)} ${product.unit} from container (now ${product.containers[0].toFixed(3)} ${product.unit})`);
        remaining = 0;
      } else {
        fromContainers.push(`${available.toFixed(3)} ${product.unit} from container (now 0 ${product.unit})`);
        remaining -= available;
    ...
    html += `<p><strong>${product.name}</strong><br>
             Rate: ${product.rate.toFixed(3)} ${product.unit}/ha &nbsp; Per Load: ${required.toFixed(3)} ${product.unit}<br>
