...
window.onload = function () {
  ...
  let currentLoad = 0;
  containerState = products.map(p => [...p.containers].sort((a, b) => a - b));
      ...
      button.onclick = () => {
        ...
        if (allConfirmed && !existingLoadedBtn) {
          ...
          loadedBtn.onclick = () => {
            ...
            tsP.textContent = `🕒 Loaded at ${ts} on ${today}`;
            loadDiv.appendChild(tsP);
            job.loadTimes.push(`${ts} on ${today}`);
...
function exportJob() {
  ...
  doc.setFontSize(11);
  doc.text(`Client: ${job.client}`, marginLeft, y); y += 6;
  ...
  doc.text(`Mixer: ${mixer}`, marginLeft, y); y += 6;
  doc.text(`Order #: ${job.orderNumber || 'N/A'}`, marginLeft, y); y += 8;

  doc.text(`Hectares: ${job.hectares}`, marginLeft, y); y += 6;
  doc.text(`Vol/Ha: ${job.volPerHa}`, marginLeft, y); y += 6;
  doc.text(`Total Vol: ${Math.round(job.totalVolume)}`, marginLeft, y); y += 6;
  doc.text(`Load Area: ${job.loadArea.toFixed(1)} | Load Vol: ${Math.round(job.loadVolume)} | Loads: ${job.loads}`, marginLeft, y); y += 10;
  ...
  const filenameSafeClient = job.client.replace(/\s+/g, '_');
  const filenameSafeOrder = job.orderNumber ? job.orderNumber.replace(/\s+/g, '_') : 'order';
  doc.save(`${filenameSafeClient}_${filenameSafeOrder}_mixer_sheet.pdf`);
}
