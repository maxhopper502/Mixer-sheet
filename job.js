...
function renderLoads() {
  ...
  loads.forEach((load, i) => {
    ...
    load.products.forEach((p, j) => {
      ...
      button.textContent = p.added ? `✅ ${p.name} (${p.volume} ${getUnit(p.name)})` : `➕ Add ${p.name} (${p.volume} ${getUnit(p.name)})`;
      button.disabled = p.added;
      button.onclick = () => {
        console.log(`Added ${p.name} (${p.volume}) to Load ${i + 1}`);
        p.added = true;
        renderLoads();
      };
