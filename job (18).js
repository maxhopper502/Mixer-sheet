...
function updateDisplay() {
  ...
  if (state.loadNumber <= state.job.loads) {
    ...
    block.innerHTML += `<button id="loadedBtn" onclick="markLoaded()" disabled>✅ Loaded</button>`;
    container.appendChild(block);
  }
  ...
  state.products.forEach(p => {
    ...
    summary.innerHTML += `<p>${p.name}: ${total} ${p.unit}</p>`;
  });
...
function markAdded(index) {
  ...
  if (added.size === state.products.length) {
    document.getElementById("loadedBtn").disabled = false;
  }
}
