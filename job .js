...
function showHeader() {
  ...
  const totalWater = (job.hectares * job.volPerHa).toFixed(1);
  el.innerHTML = `
    <p><strong>Client:</strong> ${job.client}</p>
    <p><strong>Crop:</strong> ${job.crop} | <strong>Ha:</strong> ${job.hectares} | <strong>Water/Ha:</strong> ${job.volPerHa} L</p>
    <p><strong>Total Water:</strong> ${totalWater} L | <strong>Loads:</strong> ${job.loads}</p>
...
function addLoad() {
  ...
  // Disable Add Load button until current load is marked complete
...
function markLoadComplete(loadNum, btn) {
  ...
  const parent = document.getElementById("add-load-button");
