...
function renderLoads() {
  ...
  previewCache = JSON.parse(JSON.stringify(products.map(p => [...p.containers]))); // snapshot container state
  ...
  loads.forEach((load, i) => {
    ...
    load.products.forEach((p, index) => {
      ...
      const preview = previewContainerUsage(p.name, parseFloat(p.volume), i, index);
...
function previewContainerUsage(productName, volume, loadIndex, prodIndex) {
  const containers = JSON.parse(JSON.stringify(previewCache[products.findIndex(p => p.name === productName)]));
  const messages = [];
  for (let i = 0; i < containers.length; i++) {
    ...
    if (available > 0) {
      ...
      if (available > 0) messages.push(`→ ${used.toFixed(2)} from Container ${i + 1} (${remaining.toFixed(2)} left)`);
      volume -= used;
    }
  }
  return messages;
